 ...
 * 
 * @author Chen.f 
 * @createTime：2015年9月22日 下午5:39:02 
 *
 */
public class WebUtility {
	 ...
	 * 
	 * @param str
	 * @return if str is null or empty string return false ,otherwise return
	 *         true
	 */
	...
	public static String formatTime(String seconds) {
		...
		if (hour == 0) {
			...
			time_str = hour_str + " hous " + min_str + " mins " + sec_str
					+ " secs";
		}
	...
	//CallLog页面所用时间格式换转   00：00
	public static String formatTimeForCalllog(String seconds) {
		int time = Integer.parseInt(seconds);
		DecimalFormat df = new DecimalFormat("00");
		int sec = time % 60;
		int min = (time % 3600) / 60;
